...
| `$P key <inst> <Tab\|Enter\|Space\|Escape> [--expect-chooser]` | Presses one real key and prints what has focus afterwards. `--expect-chooser` intercepts the file chooser (no native dialog opens) and reports whether the press opened it. Use it for keyboard-only paths. |
