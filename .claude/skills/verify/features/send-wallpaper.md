...
- `send-status`: the target row status becomes `delivered`.
...
- **Send.** Run `pw click a "Send" 1` (the submit button), then `pw wait a "delivered" 120`. The dropzone resets, and b's row reads `verify-b delivered`. Run `pw shot a delivered`.
...
- The receiver's `meta.filename` (shown in its Received tab) is the sender's full absolute path, not the basename. Observed 2026-09-25. Treat this as a product finding, not a verification failure.
...
- Verified end to end on 2026-09-25 (browse path, A→B, hashes matched). This is the first real smoke of the `webUtils.getPathForFile` fix that `qa-desktop.md` flags.
