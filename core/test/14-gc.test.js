...
test('gc: blobs are cleared on sender, target and relay once the target applied them', async function (t) {
  t.timeout(120000)
  ...
  const blobs = []
  for (let i = 0; i < N; i++) {
    const { id } = await a.sendWallpaper(bigPng(1024 * 1024), [b.deviceKey])
    blobs.push(await blobOf(a, id))
    await applyOn(b, id)
  }

  for (const [name, peer] of [['sender', a], ['target', b], ['relay', c]]) {
    await until(peer, 'update', async () => {
      for (const blob of blobs) if (await peer.blobs.has(blob)) return false
      return true
    }, 30000).catch(() => {})
    let held = 0
    for (const blob of blobs) if (await peer.blobs.has(blob)) held++
    t.is(held, 0, `${name} holds none of the ${N} applied blobs`)
  }
  t.is(receivedFiles(b).length, N, 'target keeps the files it applied')

  t.comment(`after ${N} x 1 MB sends a->b, all applied by b:`)
  t.comment('  du a=' + du(a.storageDir) + ' b=' + du(b.storageDir) + ' c(relay, not a target)=' + du(c.storageDir))
  t.comment('  b/received ' + du(path.join(b.storageDir, 'received')))
})
