...
- 2026-09-25 Plan 4 d1-android-ux: Five commits: clear stale joinError per attempt (#22); surface setter failures in the banner (#23); explain empty Send targets with Cancel (#24); new Sent tab with per-target status (#25); Devices mints the creator's first invite, dead Onboarding invite removed (#20 Android). Jest 77/77; emulator pass deferred to end of Step D.
- 2026-09-25 Plan 4 c1-newest-wins: _newestUnappliedForMe returns null once the newest send for this device is acked (same rule as superseded); _checkIncoming/_relayBlobs share a busy/dirty sweep gate. Live: offline device that missed two sends now ends on the newest, older reads superseded.
- 2026-09-25 Plan 4 a2-send-status: Send rows no longer carry the newest send's status; a Recently sent list shows each send's picture, time and per-target status in Android's wording. Live: picking a new picture shows no stale Delivered.
- 2026-09-25 Plan 4 a6-sender-name: core stores a basename in meta.filename (Android passes the share's display name); Received rows on both shells show From <sender> and the time, basenaming old ops defensively; desktop Re-apply confirms with 'Wallpaper set'. Live: b's view holds a6-pic1.png, not a path.
- 2026-09-25 Plan 4 a8-worker-death: worker-supervisor restarts a crashed worker with backoff (cap 5), main answers requests while it's down, renderer shows Starting and a Try again screen, quit no longer waits 2 s. Live: kill -9 -> Starting -> back to MainView; crash loop -> Try again works; quit with dead worker 0.18 s. pw launch now waits past Starting.
- 2026-09-25 Plan 4 c2-join-survives-quit: A clean close() marks the active join interrupted so pending-invite survives; _open resumes via a synchronous _startJoin so groupStatus is 'joining' at ready(). Live: joiner quit while waiting, relaunched on Waiting (joining), approved, became a member.
- 2026-09-25 Plan 4 a3-early-format-check: Picker accepts JPEG/PNG/WebP only; every pick runs core's real sniff via a new checkImage bridge command, so a HEIC (even renamed .jpg) is refused at pick time with plain advice; dropzone shows the file name, not the path. Rebase merge duplicated two requires in 08-send; fixed, and ship.sh now reruns suites after rebase.
- 2026-09-25 Plan 4 d2-worklet-lifecycle: worklet-client is the only Worklet constructor: UI claims it, background rounds lease it, release hands over; bridge wrapper is a starting/ready/dead gate so nothing hangs; one apply controller on the handle. Jest proves both (Worklet count 1, setter once); emulator pass pending at end of Step D.
- 2026-09-25 Plan 4 a4-keyboard-access: Dropzone is a real button that opens the hidden input (Space/Enter work, drops unchanged); nav marks the open tab with aria-current and a visible style; html lang=en. Live CDP keyboard walk: Tab reaches the dropzone, Space opens the chooser. pw gained a key command.
